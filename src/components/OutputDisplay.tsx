...
const DisplayItem = forwardRef<HTMLDivElement, DisplayItemProps>(({ children, label, className, ...props }, ref) => {
  return (
    <div
      ...
      ref={ref}
      className={cn(
        "pinto-sm sm:pinto-body-light text-pinto-light sm:text-pinto-light flex flex-row justify-between items-center py-2",
        className,
      )}
    >
      {label}
  ...
  ({ value, suffix = "", showArrow, token, delta, className, ...props }, ref) => {
    ...
    return (
      <div
        {...props}
        ref={ref}
        className={cn("flex flex-row items-center gap-2 pinto-body-light sm:pinto-h4 font-light", className)}
      >
        ...
        <div
          className={`${delta ? (isPositiveDelta ? "text-pinto-green-4 sm:text-pinto-green-4" : "text-pinto-error sm:text-pinto-error") : "text-pinto-primary sm:text-pinto-primary"}`}
